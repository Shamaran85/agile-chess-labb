const {
  ...
  io,
  MongoClient,
  jwt, // Import system packages
  webServer,
  dbServer, // Import custom configs
  userArgs,
  eventArgs,
  privateKey // Import custom arguments
} = require("./config");
...
const {
  ...
  searchUser,
  insertUser,
  updateUser, // Import the user route functions
  getEvents,
  searchEvent,
  insertEvent,
  updateEvent // Import the event route functions
} = require("./routes");

// Server listening port
server.listen(webServer.port, () => {
  console.log(
    `Express server with Socket.io is listening on port ${webServer.port}!`
  );
  console.log(
    `View your local API at\x1b[36m http://localhost:${webServer.port}\x1b[0m`
  );
});
...
function getCurrentUserListAndEmit(socket, dbo, userArgs) {
  try {
    dbo
      .collection(userArgs.collection)
      .find({})
      .toArray((err, users) => {
        if (!err) {
...
function getCurrentEventListAndEmit(socket, dbo, eventArgs) {
  try {
    dbo
      .collection(eventArgs.collection)
      .find({})
      .toArray((err, events) => {
        if (!err) {
